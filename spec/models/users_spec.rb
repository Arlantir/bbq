  ...
  describe '.find_for_facebook_oauth' do
    ...
    let(:access_token) do
      double(
        :access_token,
        provider: 'facebook',
        info: double(email: 'ror@worldoflosttime.ru'),
        extra: double(raw_info: double(id: '1567597593404864'))
      )
    end
    ...
    context 'when user is not found' do
      it 'returns newly created user' do
        user = User.find_for_oauth(access_token, 'facebook')

        expect(user).to be_persisted
        expect(user.email).to eq 'ror@worldoflosttime.ru'
      end
    end

    # Ситуация: юзер найден по почте
    context 'when user is found by email' do
      let!(:existing_user) { create(:user, email: 'ror@worldoflosttime.ru') }
      let!(:some_other_user) { create(:user) }

      it 'returns this user' do
        expect(User.find_for_oauth(access_token, 'facebook')).to eq existing_user
      end
    end
    ...
    context 'when user is found by provider + url' do
      let!(:existing_user) do
        create(:user, provider: 'facebook',
               url: 'https://facebook.com/1567597593404864')
      end
